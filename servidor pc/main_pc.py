...
async def upload_original_file(device_id: str, filename: str, file: UploadFile = File(...)):
    ...
    if device_id not in device_media_cache: device_media_cache[device_id] = {} # Handle case where file is from explorer
    file_bytes = await file.read()
    original_b64 = base64.b64encode(file_bytes).decode('utf-8')
    # Guardamos el original en la caché, creando la entrada si no existe
...
async def get_large_media(device_id: str, filename: str):
    ...
    try:
        ...
        media_type = "application/octet-stream" # Tipo genérico para descarga
